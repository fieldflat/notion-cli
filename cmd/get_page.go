...
// getPageCmd represents the page command
...
func init() {
	...
	getPageCmd.PersistentFlags().BoolVar(&pretty, "pretty", false, "format the json output")
	getPageCmd.PersistentFlags().IntVar(&maximum, "maximum", 5, "change the upper bound of pages number")

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// getPageCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// getPageCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
